...
4. シェルスクリプトの配置
   ...
   YourBacketName には 1. で作成したバケット名を入力\
   ...
   aws s3 cp dumpall_psql.tar.gz s3://{{YourBacketName}}/dumpall_psql_$TIME.tar.gz --profile {{YourProfileName}} --endpoint-url {{YourEndpointName}}
