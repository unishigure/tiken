...
個人で遊ぶ用の Misskey サーバ（v2023.10.0）を Vultr と Cloudflare で構築する
...
- ToDo
  ...
  - ~~Vultr で VPS サーバを構築する~~
  - ~~Cloudflare で DNS の設定~~
  - Vultr サーバ への SSH の設定
  - ~~Misskey install shell script v3.0.0 の実行~~
...
[Cloudflare](https://www.cloudflare.com/) にサインイン、ログインした後、[ダッシュボード](https://dash.cloudflare.com/)が確認できるようになるため、<br/>
...
先程購入したドメインが追加されているはずなので、該当ドメインの **管理** を選択

![dnsSetting1](/others/dnsSetting1.png)

クイックアクションの DNS の設定を更新する を選択

DNS レコードの管理画面が表示されるので、レコードの追加を選択し、以下の内容で保存する<br/>

<Callout type="info" emoji="️ℹ️">
  名前には利用したいサブドメインを設定する
  <br />
  ドメインが`example.com`だった場合、サブドメインを`mk`とすると、`mk.example.com`がサーバーのURLとなる
  <br />
